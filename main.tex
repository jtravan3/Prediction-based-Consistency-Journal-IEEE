...
\begin{proposition}
\label{prop:elevate}
Let $H$ be a history produced by a prediction-based scheduler. If $p_{1}[x]$ requires an eleveate action ($\delta$) (outlined in Table \ref{tbl:read_lock_compatibility} \& Table \ref{tbl:write_lock_compatibility}) then there exists one or many $q_{i}[x]$ where $q_{i}[x]$ $\subseteq$ $\{Q:q_{1}[x],q_{2}[x],..,q_{n}[x]\}$ and $Q$ $\subseteq$ $C(H)$. Therefore $\forall$ $q_{i}[x]$ in $Q$ then $qu_{i}[x] < pl_{1}[x]$.
\end{proposition}
